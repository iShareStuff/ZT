...
class Utils extends AddonModule {
  ...
  public async createBackupZIP(isExport = false) {
    ...
    await this._Addon._Zotero.File.createDirectoryIfMissingAsync(
      this._Addon._Zotero.Prefs.get("tara.exportPath")
    );
      ...
      try {
        if (task == "preferences") {
          ...
          let saveDir = isExport
            ...
            : tmpDir;
          await this._Addon._Zotero.File.createDirectoryIfMissingAsync(saveDir);
          ...
          let tmpPath =
            this._Addon._Zotero.Prefs.get("tara.exportPath") + "tmp";
          await this._Addon._Zotero.File.createDirectoryIfMissingAsync(tmpPath);
          let zipfile: string = OS.Path.join(tmpPath, "backup.zip");
        ...
        } else if (task == "exportProfile") {
          this._Addon._Zotero.debug("** exportProfile");
          let exportProfilePath =
            this._Addon._Zotero.Prefs.get("tara.exportPath") + "ZoteroProfile";
          //if (!(await OS.File.exists(exportProfilePath))) {
          //OS.File.makeDir(exportProfilePath);
          //}
          await this._Addon._Zotero.File.createDirectoryIfMissingAsync(
            exportProfilePath
          );
          await this._Addon._Zotero.File.iterateDirectory(
            ...
            function (entry) {
              ...
              if (entry.name != "parent.lock") {
                // 不为parent.lock则复制
                var dest_profile = OS.Path.join(exportProfilePath, entry.name);
                if (entry.isDir) {
                  this._Addon._Zotero.File.copyDirectory(
                    entry.path,
                    dest_profile
                  );
                } else {
          ...
          this._Addon._Zotero.debug("** exportData");
          let exportDataPath =
            this._Addon._Zotero.Prefs.get("tara.exportPath") + "ZoteroData";
          await this._Addon._Zotero.File.createDirectoryIfMissingAsync(
            exportDataPath
          );
          await this._Addon._Zotero.File.copyDirectory(dataDir, exportDataPath);
        } else if (task == "exporting") {
          this._Addon._Zotero.debug("** exporting");
        } else if (task == "exportSuccess") {
          this._Addon._Zotero.debug("** exportSuccess");
        }
        let pvalue: string = (
          (1 - this._Addon.views.queue.length / totalTasks) *
          100
        ).toString();
  ...
  public async exportDirOfProfile() {
    this._Addon._Zotero.debug("** Tara start export export dir of profile");
    let queue = {
      exporting: true,
      exportProfile: this._Addon._Zotero.Prefs.get("tara.exportProfile"),
      exportSuccess: true,
    };
    this._Addon.views.queue = Object.keys(queue).filter((k) => queue[k]);
    await this.createBackupZIP(true);
    this._Addon._Zotero.debug("** Tara finish export dir of profile");
  }

  public async exportDirOfData() {
    this._Addon._Zotero.debug("** Tara start export dir of Data");
    let queue = {
      exporting: true,
      exportData: this._Addon._Zotero.Prefs.get("tara.exportData"),
      exportSuccess: true,
    };
    this._Addon.views.queue = Object.keys(queue).filter((k) => queue[k]);
    await this.createBackupZIP(true);
    this._Addon._Zotero.debug("** Tara finish export dir of data");
  }
}
