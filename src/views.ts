...
class AddonViews extends AddonModule {
  ...
  public initViews(_Zotero) {
    ...
    var exportProfile_menu = _window.document.createElement("menuitem");
    var exportData_menu = _window.document.createElement("menuitem");
    ...
    const exportProfileLabel = this._Addon.locale.getString(
      "toolbar.exportProfile"
    );
    const exportDataLabel = this._Addon.locale.getString("toolbar.exportData");
    ...
    exportProfile_menu.setAttribute("id", "zotero-tb-tara-export-profile");
    exportProfile_menu.setAttribute("label", exportProfileLabel);
    exportProfile_menu.setAttribute("class", "menuitem-iconic");
    exportProfile_menu.setAttribute(
      "style",
      "list-style-image: url('chrome://tara/skin/export_icon.png');"
    );
    exportProfile_menu.setAttribute(
      "oncommand",
      "Zotero.Tara.utils.exportDirOfProfile();"
    );

    exportData_menu.setAttribute("id", "zotero-tb-tara-export-data");
    exportData_menu.setAttribute("label", exportDataLabel);
    exportData_menu.setAttribute("class", "menuitem-iconic");
    exportData_menu.setAttribute(
      "style",
      "list-style-image: url('chrome://tara/skin/export_icon.png');"
    );
    exportData_menu.setAttribute(
      "oncommand",
      "Zotero.Tara.utils.exportDirOfData();"
    );

    menupopup.appendChild(create_menu);
    menupopup.appendChild(export_menu);
    menupopup.appendChild(restore_menu);
    menupopup.appendChild(exportProfile_menu);
    menupopup.appendChild(exportData_menu);
  ...
  public updateProgressWindow(
    ...
    if (status) {
      innerHTML = `<img src="${this.tickIcon}"> ${this._Addon.locale.getString(
        row
      )}`;
    } else {
      innerHTML = `<img src="${this.crossIcon}"> ${this._Addon.locale.getString(
        row
      )}`;
    }
  ...
  public completeProgressWindow(isExport: boolean) {
    ...
    if (isExport) {
      doc.querySelector("#msg").textContent =
        this._Addon.locale.getString("complete.export") +
        this._Addon._Zotero.Prefs.get("tara.exportPath");
    } else {
      doc.querySelector("#msg").textContent =
        this._Addon.locale.getString("complete.msg");
    }
  }
}
